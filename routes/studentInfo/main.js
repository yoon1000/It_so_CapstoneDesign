...
router.post('/semester', function(req, res, next) {
    var sql = 'select subject_name, required, credit from majorsubject where subject_name NOT IN (select subject_name from Student_majorsubject where id = ?) AND major = ? AND semester REGEXP(?)';
