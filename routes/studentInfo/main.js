...
router.post('/semester', function(req, res, next) {
    ...
    var sql = 'select subject_name from majorsubject where subject_name NOT IN (select subject_name from Student_majorsubject where id = ?) AND major = ? AND semester = ?';
    mysqlDB.query(sql, [req.body.id, req.body.major, semester], function(error, result) {
        if(error == null) {
            res.json({
                "code" : 200,
                "result" : result
