...
router.post('/semester', function(req, res, next) {
    var sql = 'select m.subject_name, m.required, m.credit from majorsubject as m where subject_name NOT IN (select s.subject_name from Student_majorsubject as s where s.id = ?) AND m.major = ? AND m.semester REGEXP(?)';
